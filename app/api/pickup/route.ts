...
export async function POST(req: Request) {
  ...
  try {
    ...
    const cookies = req.headers.get("cookie") || "";
    let guestSessionId = getCookie(cookies, "guest_session_id");

    // Sanitize and validate guestSessionId
    if (guestSessionId) {
      guestSessionId = guestSessionId.trim();
      // Basic UUID v4 validation
      if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(guestSessionId)) {
        console.warn("Invalid guestSessionId format, ignoring:", guestSessionId);
        guestSessionId = null;
      }
    }
    ...
    // Handle guest session: ensure record exists
    if (guestSessionId && !session?.user?.id) {
      console.log("Ensuring guest session exists:", guestSessionId);
      await db.guestSession.upsert({
        where: { id: guestSessionId },
        create: {
          id: guestSessionId,
    ...
    // 🔒 Recalculate cost on server — ignore client-provided cost
    const totalCost = data.packages.reduce((sum: number, pkg: any) => {
      const calc = costCalculations({
        weight: pkg.weight.toString(),
        length: pkg.dimensions.length.toString(),
    ...
    const pickup = await db.pickup.create({
      data: {
        ...
        // ⚠️ Removed `itemIds` — items are not created yet, so linking fails
        packages: {
          create: data.packages.map((pkg: any) => ({
            packageType: pkg.packageType,
            weight: pkg.weight,
            ...
            height: pkg.dimensions?.height,
            specialNotes: pkg.specialNotes,
            // itemIds: pkg.itemIds, ← REMOVED
          })),
        },
        commercialDocuments: data.commercialDocuments || null,
        calculatedCost: totalCost, // ✅ Trust only server cost
        costCurrency: data.costCurrency || "USD",
      },
      include: {
        items: true,
