...
export const WarehousesList = ({ onAddNew, onEdit }: WarehousesListProps) => {
  ...
  const getActivePickups = (warehouse: Warehouse) => {
    ...
    const outbound = warehouse.outboundPickups ?? [];
    const inbound = warehouse.inboundPickups ?? [];
    const outboundActive = outbound.filter(p => activeStatuses.includes(p.status)).length;
    const inboundActive = inbound.filter(p => activeStatuses.includes(p.status)).length;
    return outboundActive + inboundActive;
  };

  if (loading) return <div className="flex justify-center p-6">Loading warehouses...</div>;
  if (error) return <div className="text-red-500 p-4 text-center">{error}</div>;
    ...
    <div>
      <Card>
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <CardTitle className="text-lg">Warehouses Management</CardTitle>
          ...
          <div className="space-y-4">
            {warehouses.length === 0 ? (
              ...
              warehouses.map((warehouse) => (
                ...
                >
                  {/* Top Row: Status + Name + Stats */}
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                    <div className="flex flex-col flex-1 min-w-0">
                      <span
                        className={`inline-block px-2 py-0.5 text-xs rounded w-fit ${warehouse.status === 'ENABLED'
                      ...
                      <h3 className="font-semibold text-base truncate mt-1">{warehouse.name}</h3>
                    </div>
                    <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1">
                        <Package className="w-3.5 h-3.5" />
                        <span>{getTotalPickups(warehouse)} total</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                        <span>{getActivePickups(warehouse)} active</span>
                  ...
                  <div className="flex gap-2 mb-3">
                    ...
                    <div className="text-sm text-muted-foreground min-w-0">
                      <p className="truncate">{warehouse.address.line1}</p>
                      {warehouse.address.line2 && <p className="truncate">{warehouse.address.line2}</p>}
                      <p className="truncate">
                        {warehouse.address.city}, {warehouse.address.state} {warehouse.address.postalCode}
                      </p>
                      <p className="text-xs">{warehouse.address.country}</p>
                    </div>
                  </div>
                  ...
                  {/* Footer: Created + Active Badge + Actions */}
